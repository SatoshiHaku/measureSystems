...
class NetAnaProcedure(Procedure):
    ...
    freq_cw = IntegerParameter("cw frequency",units="Hz",default=4.75e8)
    power = IntegerParameter("power",units="dbm",default=10)
    sweeptime = FloatParameter("sweep time",units="s",default=200)#ポイント数をあげるとこれを短くしないとだめ　なぜ？
    points = IntegerParameter("number of points", default=200)
...
# averaging??
    ...
    DATA_COLUMNS = ['frequency','Vol',"angle","magnetic field","S21 or S12"]#must be same names to data columns
    ...
    def execute(self):
        ...
        for i in range(self.anglepoints):
            ...
            data = {
                ...
                "angle":deltaAngle,
                'Vol': nanovol.measure_voltage(),
                "magnetic field": self.RateMH*self.voltage,
                "direction":self.Spara
...
class MainWindow(ManagedDockWindow):
    ...
    def __init__(self):
        super().__init__(
            ...
            inputs=['delay',"freq_cw","power","sweeptime","points","anglepoints","voltage","RateMH","Spara"],#must be all input columns
            displays=['delay',"freq_cw","power","sweeptime","points","voltage","RateMH","Spara"],#include in progress
            x_axis=['angle'],#default(must be in data columns) if you use ManagedDockWindow, use list of columns
            y_axis=["Vol"],
            sequencer=True,
            sequencer_inputs=['delay',"freq_cw","power","sweeptime","points","anglepoints","voltage","RateMH","Spara"],
